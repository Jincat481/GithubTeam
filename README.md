...
* 이펙트 시스템이 들어가서 문제인지 콜라이더 자체로 충돌처리는 되지 않고 파티클 시스템 내에 콜리젼을 키고 코드내에서 OnParticleCollision(GameObject 변수이름)을 활용하여 데미지 및 파괴 구현, Radius Scale 0.05로 설정해서 트리거와 비슷하게 제작
---
# 몬스터 스포너 구현 사항
## 2024/08/02 현재 프로토타입 버전
